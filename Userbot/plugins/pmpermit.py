...
DEFAULT_TEXT = "❏ ʜᴀɪ {Mention} 👋\n└ sᴀʏᴀ ᴀsɪsᴛᴇɴ ǫᴜᴀᴅʀᴇxᴜs\n\nsᴀʏᴀ ᴅɪʀᴀɴᴄᴀɴɢ ᴜɴᴛᴜᴋ ᴍᴇɴᴊᴀɢᴀ ʀᴏᴏᴍ ᴄʜᴀᴛ ɪɴɪ.\nᴛᴏʟᴏɴɢ {Mention} ᴛɪᴅᴀᴋ ᴍᴇʟᴀᴋᴜᴋᴀɴ sᴘᴀᴍ, ᴛᴜɴɢɢᴜ ᴍᴀᴊɪᴋᴀɴ sᴀʏᴀ ᴀᴋᴛɪғ ᴀᴛᴀᴜ ᴀɴᴅᴀ ᴀᴋᴀɴ ᴏᴛᴏᴍᴀᴛɪs ᴛᴇʀʙʟᴏᴋɪʀ!!\n\nᴄʜᴀɴɴᴇʟ : @qdrxsmanagement"
...
@ky.ubot("pmpermit|antipm")
async def _(c: nlx, m, _):
    em = Emojik(c)
    em.initialize()
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    jing = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    try:
...
async def _(c: nlx, m, _):
    ...
    if chat_type in [ChatType.GROUP, ChatType.SUPERGROUP]:
        ...
        return await m.delete()
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    babi = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
