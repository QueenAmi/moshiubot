...
async def _(c, m, _):
    ...
    ceger = await m.reply_text(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    try:
...
async def _(c, m, _):
    ...
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    ceger = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    try:
...
async def _(c: nlx, m, _):
    ...
    em.initialize()
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    xenn = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
...
async def _(c: nlx, m, _):
    ...
    em.initialize()
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    pp = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    chat_id = m.command[1] if len(m.command) > 1 else m.chat.id
...
async def _(c: nlx, m, _):
    ...
    em.initialize()
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    msg = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
...
async def _(c, m, _):
    ...
    em.initialize()
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    xenn = await m.reply_text(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    luci = 0
