...
from Userbot.helper.tools import Emojik, ReplyCheck, h_s, initial_ctext, ky

__MODULES__ = "•AlQur'an•"
USER_PREMIUM = True


def help_string(org):
    return h_s(org, "help_quran")
...
async def _(c: nlx, m, _):
    ...
    if not surah_name:
        await m.reply(_("qur_1").format(em.gagal))
        await pros.delete()
        return
    ...
    if surah_info:
        ...
        if audio_url:
            ...
            if download_audio(audio_url, audio_file_name):
                ...
                await m.reply(_("qur_2").format(em.gagal))
        else:
