...
async def _(c: nlx, m, _):
    ...
    tex = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    if not user_id:
...
async def _(c: nlx, m, _):
    ...
    em.initialize()
    pong_, uptime_, owner_, ubot_, proses_, sukses_ = initial_ctext(c)
    bacot = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    a_chats = []
    me = await c.get_me()
...
async def _(c: nlx, m, _):
    ...
    try:
        pat = await replied.download()
        if replied.photo:
            await c.set_profile_photo(photo=pat)
        elif replied.video:
            await c.set_profile_photo(video=pat)
        os.remove(pat)
        return await m.reply(_("prof_9").format(em.sukses))
    except Exception as e:
