...
async def _(c: nlx, m, _):
    ...
    ky = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    ...
    try:
        ...
        await ky.edit(_("err").format(em.gagal, e))
        return
...
async def _(c: nlx, m, _):
    ...
    await ky.edit(_("vc_3").format(em.sukses))
    return
...
async def _(c: nlx, m, _):
    ...
    ky = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    if len(m.command) < 2:
        await ky.edit(_("vc_4").format(em.gagal, m.command))
        return
    if not (group_call := (await get_group_call(c, m, _, err_msg=", Error..."))):
        return
    try:
        ...
        await ky.edit(_("vc_5").format(em.gagal))
        return
    await ky.edit(_("vc_6").format(em.sukses, txt))
    return
...
async def _(c: nlx, m, _):
    ...
    if chat_id:
        try:
            ...
            await ky.edit(_("vc_9").format(em.sukses, title))
            return
        except NotInCallError:
            return await ky.edit(_("vc_13").format(em.gagal, title))
        except Exception as e:
            await ky.edit(_("err").format(em.gagal, e))
            return
    else:
