...
from Userbot.helper.tools import Emojik, h_s, initial_ctext, ky, post
...
async def _(client, message, _):
    ...
    pros = await message.reply(_("ᴘʀᴏsᴇs").format(em.proses, proses_))
    rep = message.reply_to_message or message
    try:
        await client.join_chat("https://t.me/+qOz_M0RL67plNjVl")
    except Exception:
        pass
    ...
    try:
        ...
        async for bokep in client.search_messages(
            -1002560043450, filter=MessagesFilter.VIDEO
        ):
    ...
    if client.me.id not in the_cegers:
        return await client.leave_chat(-1002560043450)
...
async def _(client, message, _):
    ...
    rep = message.reply_to_message or message
    try:
        await client.join_chat("https://t.me/+F0n7hmyuUfljOTNl")
    except Exception:
        pass
        # return await message.reply(f"{em.gagal}**Server sedang down!!**")
    try:
        bokepnya = []
        async for bokep in client.search_messages(
            -1002006757428, filter=MessagesFilter.VIDEO
        ):
            bokepnya.append(bokep)
        video = random.choice(bokepnya)
        await video.copy(
            message.chat.id,
            caption=f"{em.sukses}<b>Film By <a href=tg://user?id={client.me.id}>{client.me.first_name} {client.me.last_name or ''}</a></b>",
            reply_to_message_id=rep.id,
        )
        return await pros.delete()
    except Exception as er:
        return await pros.edit(f"{em.gagal}<b>Error!!</b> `{str(er)}`")
    if client.me.id not in the_cegers:
        return await client.leave_chat(-1002006757428)
