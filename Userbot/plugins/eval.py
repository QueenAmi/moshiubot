...
from Userbot.helper.tools import Emojik, fetch, ky, paste
...
async def _(c: nlx, m, _):
    ...
    return await xx.edit(f"<pre>{softw}</pre>")
...
async def _(c: nlx, m, _):
    ...
    if not m.reply_to_message:
        return
    pros = await m.reply(_("ᴘʀᴏsᴇs").format(em.proses))
