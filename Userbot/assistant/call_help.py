...
from Userbot.helper.tools import (INLINE, Button, cek_tg,
                                  ...
                                  ky, org_kontol, paginate_modules,
...
async def reset_costumtext(c, m, _):
    ...
    dB.set_var(m.from_user.id, "text_pong", "ᴘᴏɴɢ")
    dB.set_var(m.from_user.id, "text_uptime", "ᴜᴘᴛɪᴍᴇ")
    mmg = f"<a href=tg://user?id={m.from_user.id}>{m.from_user.first_name} {m.from_user.last_name or ''}</a>"
    dB.set_var(m.from_user.id, "text_owner", f"ᴏᴡɴᴇʀ: {mmg}")
    dB.set_var(m.from_user.id, "text_ubot", f"{nama_bot}")
    dB.set_var(m.from_user.id, "text_gcast", "ᴘʀᴏsᴇs")
    dB.set_var(m.from_user.id, "text_sukses", "ɢᴄᴀsᴛ sᴜᴋsᴇs")
...
@ky.regex(
    r"^(📜 Buat Userbot|📇 Status Akun|🛠️ Cek Fitur|🇲🇨 Bahasa|🧸 Owner|🚀 Updates|👥 Cek User|✅ Setuju|❌ Tidak|🏠 Menu Utama|📃 Saya Setuju|🛠️ Cek Fitur|Start|start|✅ Lanjutkan Buat Userbot|🆘 Dukungan|🔄 Reset Emoji|🔄 Reset Costum Text|🔄 Reset Prefix|🔄 Restart Userbot|⬅️ Kembali)",
    human.pv,
)
@org_kontol
async def _(c, m, _):
...
async def _(c, cq, _):
    ...
    if mod_match:
        ...
        try:
            ...
            return await costum_cq(
                **{
                    ...
                    + f"<blockquote><b>🤖 {nama_bot}</b></blockquote>"
                },
                reply_markup=button,
            )
        ...
        try:
            return await costum_cq(
                **{costum_text: top_text.format(" ".join(prefix), len(CMD_HELP), full)},
