...
from Userbot.helper.tools import MSG, Ads, Button, ky, setting_emoji
...
async def cb_bayar_dulu(client, m, _):
    msg = f"""
<b><blockquote>✅ Jika anda sudah setuju dengan syarat & ketentuan, Silahkan hubungi 🧸 @MOSHI404 untuk mendapatkan akses membuat userbot.</b></blockquote>
<b>Ads: {Ads()}</b>"""
...
async def bikin_ubot(client, m, _):
    ...
    if user_id in nlx._my_id:
        return await bot.send_message(
            user_id,
            f"<b><blockquote>Anda telah memasang userbot</blockquote></b>",
    ...
    if not user_id == kn_client.me.id:
        ...
        return await bot_msg.edit(
            f"<blockquote><b>Gunakan akun anda sendiri, bukan orang lai!!</b></blockquote>\n<b>Ads: {Ads()}</b>"
        )
...
@ky.callback("^(get_otp|get_phone|get_faktor|ub_deak|deak_akun)")
