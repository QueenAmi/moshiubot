################################################################
"""
MoshixUbot Open Source . Maintained ? Yes Oh No Oh Yes Ngentot
...
async def all_inline(__HELPERS__, q, _):
    answerss = [
        InlineQueryResultArticle(
            ...
            thumb_url=log_pic,
            reply_markup=ikb([[("Click Here", f"https://t.me/{bot_username}", "url")]]),
        )
    ]
...
async def inline_help(j, iq, _):
    ...
    full = f"<a href=tg://user?id={iq.from_user.id}>{iq.from_user.first_name} {iq.from_user.last_name or ''}</a>"
    msg = "**Commands\n      Prefixes: `{}`\n      Modules: `{}`\n      User: {}**".format(
        " ".join(prefix), len(CMD_HELP), full
    )
...
<b>UserInfo:</b>
   <b>name:</b> <b>{cek['name']}</b>
      <b>id:</b> <code>{cek['id']}</code>
      <b>created:</b> <code>{cek['create']}</code>
...
<b>{nama_bot}</b>
    <b>status:</b> {stutas} 
      ...
      <b>ubot_uptime:</b> <code>{upnya}</code>
