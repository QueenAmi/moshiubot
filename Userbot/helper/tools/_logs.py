...
def initial_ctext(c):
    pong_ = dB.get_var(c.me.id, "text_pong") or "ᴘᴏɴɢ"
    uptime_ = dB.get_var(c.me.id, "text_uptime") or "ᴜᴘᴛɪᴍᴇ"
    mmg = f"<a href=tg://user?id={c.me.id}>{c.me.first_name} {c.me.last_name or ''}</a>"
    owner_ = dB.get_var(c.me.id, "text_owner") or f"ᴏᴡɴᴇʀ: {mmg}"
    ubot_ = dB.get_var(c.me.id, "text_ubot") or f"{nama_bot}"
    proses_ = dB.get_var(c.me.id, "text_gcast") or "ᴘʀᴏsᴇs"
    sukses_ = dB.get_var(c.me.id, "text_sukses") or "ɢᴄᴀsᴛ sᴜᴋsᴇs ᴍᴇᴋ"
    return pong_, uptime_, owner_, ubot_, proses_, sukses_
